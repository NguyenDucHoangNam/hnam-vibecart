...
public class ProductController {
        ...
        public ResponseEntity<ApiResponse<ProductResponse>> create(@Valid @RequestBody ProductRequest request) {
                ...
                return ResponseEntity.status(HttpStatus.CREATED).body(
                                ApiResponse.<ProductResponse>builder().code(1000).message("Tạo sản phẩm thành công")
                                                .result(product)
                                                .build());
        }
        ...
        public ResponseEntity<ApiResponse<ProductResponse>> getById(@PathVariable String id) {
                ...
                return ResponseEntity.ok(
                                ApiResponse.<ProductResponse>builder().code(1000)
                                                .message("Lấy thông tin sản phẩm thành công")
                                                .result(product).build());
        }

        @GetMapping
        public ResponseEntity<ApiResponse<SearchResultResponse>> getAll(ProductSearchRequest request) {
        ...
        public ResponseEntity<ApiResponse<Page<ProductResponse>>> getByCreator(
                ...
                return ResponseEntity.ok(
                                ApiResponse.<Page<ProductResponse>>builder().code(1000)
                                                .message("Lấy sản phẩm của Creator thành công")
                                                .result(products).build());
        }
        ...
        public ResponseEntity<ApiResponse<Void>> adjustInventory(
                ...
                return ResponseEntity.ok(
                                ApiResponse.<Void>builder().code(1000).message("Điều chỉnh tồn kho thành công")
                                                .build());
        }
        ...
        public ResponseEntity<ApiResponse<java.util.List<com.vibecart.api.modules.ecommerce.dto.response.InventoryHistoryResponse>>> getInventoryHistory(
                        @PathVariable String variantId) {
                java.util.List<com.vibecart.api.modules.ecommerce.dto.response.InventoryHistoryResponse> history = productService
