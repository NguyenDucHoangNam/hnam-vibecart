...
### E. Quy trình Xác nhận tải lên & Dọn dẹp tệp tin rác (Multi-Layer Cleanup)
...
* **Giải pháp (3 lớp phòng vệ phối hợp)**:
  ...
  * **Lớp 1 - Active Cleanup (Dọn dẹp chủ động tại `/confirm`)**: Khi API `/confirm` phát hiện tệp tin không hợp lệ (dung lượng thực tế trên S3 không khớp hoặc tệp không tồn tại), hệ thống sẽ **chủ động xóa ngay lập tức** cả tệp rác trên S3 lẫn bản ghi `PENDING` mồ côi trong DB trước khi trả về lỗi cho Client. Hành động xóa S3 được bao bọc trong khối `try-catch` riêng biệt để đảm bảo quy trình dọn dẹp không bị crash nếu S3 gặp sự cố.
  * **Lớp 2 - Tác vụ dọn dẹp định kỳ (MediaCleanupScheduler)**: Xử lý các trường hợp Client không bao giờ gọi `/confirm` (tắt trình duyệt, mất mạng, bỏ cuộc). Một Background Job chạy định kỳ mỗi 15 phút sẽ tự động tìm tất cả các bản ghi có trạng thái `PENDING` được tạo cách đây quá 15 phút, tiến hành xóa tệp tin trên S3 và xóa bản ghi DB tương ứng một cách an toàn.
  * **Lớp 3 - S3 Lifecycle Rules (Lưới an toàn hạ tầng)**: Cấu hình quy tắc vòng đời trực tiếp trên S3/MinIO để tự động xóa các tệp tin trong thư mục tạm sau một khoảng thời gian cố định (ví dụ: 24 giờ). Đây là tuyến phòng thủ cuối cùng hoạt động hoàn toàn độc lập với Backend, đảm bảo không có tệp rác tồn tại vĩnh viễn kể cả khi Backend bị sập.
...
### E. Logic xử lý xác nhận tải lên với Active Cleanup (`MediaController.java`)
```java
@PostMapping("/confirm")
...
4. **Cấu hình CORS Policy trên S3 Bucket**:
   ...
   ```
5. **Cấu hình S3 Lifecycle Rules (Lưới an toàn dọn dẹp tệp rác)**:
   Thiết lập quy tắc vòng đời trên S3/MinIO để tự động xóa tệp rác trong thư mục tạm. Đây là tuyến phòng thủ cuối cùng ở tầng hạ tầng, hoạt động độc lập với Backend:
   * **Trên AWS S3 Console**: Vào tab **Management** → **Lifecycle rules** → **Create lifecycle rule**:
     * Rule name: `Delete-Temp-Files-After-24h`
     * Prefix filter: `temp/`
     * Actions: Tích chọn `Expire current versions of objects` và `Delete expired object delete markers or incomplete multipart uploads`.
     * Days after object creation: `1` (tự động xóa file sau 24 giờ).
   * **Trên MinIO** (môi trường local) sử dụng MinIO Client CLI:
     ```bash
     mc alias set myminio http://localhost:9000 minio_admin minio_password
     mc ilm rule add myminio/vibecart-media --expire --days 1 --prefix "temp/"
     ```
